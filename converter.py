...
class GeoJSONConverter:
	...
	def to_geojson(self, raw_geofile, raw_geodir='geojson', shapefile_prefix='', shapefile_ext='zip', shapefiles_dir='shapefiles'):
		if not isdir(raw_geodir):
			mkdir(raw_geodir)

		print "Converting files in %s matching %s*.%s into GeoJSON. Results stored in %s/%s" % (shapefiles_dir, shapefile_prefix, shapefile_ext, raw_geodir, raw_geofile)
		...
		with open(raw_geofilepath,'a') as out:
			...
			for filename in files:
				try:
					...
					print "Error converting %s" % zfile
